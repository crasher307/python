...
read = input('Введите номера телефонов через пробел: ').split()
if not read:
    read = '+71234567890 +71234567854 +61234576890 +52134567890 +21235777890 +21234567110 +71232267890'.split()
phones = {item[:-10]: list(filter(lambda f: item[:-10] == f[:-10], read)) for item in read}
print(*sorted(phones.items()))

# phones = [[item[:-10], item] for item in read]
# result = {item[0]: [] for item in phones}
# for item in phones:
